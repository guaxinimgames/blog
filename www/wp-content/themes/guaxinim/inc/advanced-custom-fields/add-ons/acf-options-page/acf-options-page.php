...
class acf_options_page_plugin
{
	...
	*/
	...
	function admin_menu() 
	{
		...
		// Parent
		$parent_page = add_theme_page($parent_title, $parent_menu, $this->settings['capability'], $parent_slug, array($this, 'html'));	
		...
		if( !empty($this->settings['pages']) )
		{
			foreach($this->settings['pages'] as $c)
			{
				...
				$child_page = add_theme_page($parent_slug, $sub_title, $sub_title, $this->settings['capability'], $sub_slug, array($this, 'html'));
