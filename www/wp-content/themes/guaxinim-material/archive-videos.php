<!DOCTYPE html><?php $taxonomy = 'videos_categories'; ?><?php $sub_header_items = isset($sub_header_items)? $sub_header_items : get_terms(array('taxonomy' => $taxonomy)); ?><?php $active_category = isset($active_category)? $active_category : $wp_query->get_queried_object(); ?>
<head>
                ...
                <div class="col-md-9">
                  <div class="highlights">
                    <h1>Últimos Vídeos</h1>
                            <div class="home-video"><?php $video = reset(get_posts(
	array(
		'posts_per_page' => 1,
		'orderby'	=> 'date',
		'order' => 'DESC',
		'post_type' => 'videos'
                              ...
                              <div class="col-md-9">
                                <div class="main-video"><?php $img = wp_get_attachment_image_src(get_post_thumbnail_id($video->ID), 'full');
if(is_array($img)) {
	$img =reset($img);
} ?>
                                  <div class="video-container">
                                    <input type="hidden" data-video="<?php echo get_field('video_link', $video->ID) ?>">
                                    <div style="<?php echo "background-image:url({$img})" ?>" class="video-overlay"></div>
                                    <div class="video"></div>
                                  </div>
                                  <div class="text"><?php $post_type = get_post_type_object(get_post_type($video->ID));
$cat = get_the_terms($video, 'videos_categories');
if(is_array($cat)) {
	$cat = reset($cat);
	$cat = " / " . $cat->name;
} ?>
                              ...
                              <div class="col-md-3">
                                <div class="related-videos"><?php $videos = get_field('related_vidoes', $video) ?><?php foreach($videos as $video): ?>
                                  <div class="video-content"><?php $img = wp_get_attachment_image_src(get_post_thumbnail_id($video->ID), 'thumbnail');
if(is_array($img)) {
	$img =reset($img);
} ?>
                                    <div class="video-container">
              ...
              <div class="row">
                <div class="col-md-9"><?php if (have_posts()): while (have_posts()): the_post() ?><?php $posts_class = "col-md-4" ?><?php $type = isset($types[$i])? $types[$i] : 'list';
...
$cat = reset(get_the_terms($post, $post_type->name . '_categories')); ?>
                  ...
                  </article><?php endif ?><?php wp_reset_postdata(); ?><?php endwhile;endif ?>
                </div>
